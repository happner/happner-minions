...
Master.prototype.loadScript = function($happn, scriptName, opts, callback) {
  ...
  try {
    var script = require(process.cwd() + '/scripts/' + scriptName + '_script');
...
Master.prototype.viewScript = function($happn, opts, callback) {
  ...
  this.listMinions($happn, opts, function(e, minions) {
    ...
    for (var minionName in minions) {
      ...
      try {
        var errs = minion.metrics.gauges.errors;
        if (typeof errs == 'number') errors += errs;
      } catch (e) {}
      ...
      if (task.startedAt && task.stepsRemaining == 0) {
...
Master.prototype.spawnMinions = function($happn, count, opts, callback) {
  ...
  // console.log('limits', JSON.stringify(this.limits,null,2));
...
Master.prototype.updateMinion = function($happn, update, callback) {
  ...
  $happn.log.$$DEBUG('updateMinion \'%s\'', name, minion.status);
  ...
  if (minion.status == 'done') {
    ...
    _this.listMinions($happn, {tag: minion.tag}, function(e, minions) {
      var done = true;
      for (var name in minions) {
        if (minions[name].status != 'done' && minions[name].status != 'finished') {
          done = false;
          break;
        }
      }
      if (done) {
        ...
        for (var name in minions) {
          if (minions[name].status == 'finished') continue;
          $happn.emit('minion/finish/' + name, {tag: minion.tag});
        }
      }
    });
  }
...
Master.prototype.minionFinishDone = function($happn, update, callback) {
  ...
  minion.finishResult = update.finishResult;
  _this.listMinions($happn, {tag: minion.tag}, function(e, minions) {
    var finished = true;
    for (var name in minions) {
      if (minions[name].status != 'finished') {
        finished = false;
        break;
      }
    }
    if (finished) {
      $happn.emit('minions/finished', {tag: minion.tag, finishedAt: Date.now()});
    }
  });
...
Master.prototype.minionResetDone = function($happn, update, callback) {
  ...
  delete minion.task.startedAt;
  delete minion.task.result;

  this.pendingResetMinions($happn, {tag: minion.tag, type: minion.type, script: minion.script}, function(e, result) {
    if (result.pending > 0) return;

    _this.listMinions($happn, {tag: minion.tag, type: minion.type, script: minion.script, status: 'ready'}, function(e, minions) {
...
Master.prototype.minionMetrics = function($happn, update, callback) {

  var marshal = this.marshals[update.marshalName];
  var minion = marshal.minions[update.minionName];

  minion.metrics = update.metrics; // overwrite...!
  $happn.emit('minion/metrics', update);

  callback();

}

Master.prototype.marshalMetrics = function($happn, update, callback) {

  var marshal = this.marshals[update.marshalName];


  if (!marshal) return callback();
  marshal.metrics = update.metrics; // overwrite...!
  
  $happn.emit('marshal/metrics', update);
